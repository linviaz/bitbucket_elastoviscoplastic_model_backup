...
def plot_all(sol,name,index=0):
	...
	sol_E_e = -(2.*sol.get_strain().T[0]+1.0-sol.get_inelastic_tens().T[0,2])
	
	#stress-strain
	ax[0].set_xlim(0.0, 0.05)
	ax[0].set_ylim(-2.0, 10.0)
	#print(E_11_ana-ol.get_strain().T[0])
	ax[0].plot(-E_11_ana,-Sig_11_ana,ls='-',label='Analytical',color='b')#,marker = 'o')
	ax[0].plot(-sol.get_strain().T[0],-sol.get_stress().T[0],label='Numerical $\dot{\lambda}$ = -0.0005 / min',ls='',marker = '*', color='y')
	#ax[0].plot(-sol2.get_strain().T[0],-sol2.get_stress().T[0],label='$\dot{\lambda}$ = -0.0001 /s',ls='',marker = '*',color='y')
	...
	ax[1].plot(time_ana,-E_11_ana,ls='-',label='Analytical',color='b')#,marker = 'o')
	ax[1].plot(sol.get_time(),-sol.get_strain().T[0],marker='*',label='Numerical',ls='',color='y')
	#ax[1].plot(sol2.get_time(),-sol2.get_strain().T[0],marker='*',label='$\dot{\lambda}$ = -0.0001 /s',ls='',color='y')
	#ax[1].plot(sol3.get_time(),sol3.get_stress().T[0],ls='-.',label='$\lambda$ = 0.95',color='b')
	commonFormat(ax[1],0.,0.,'upper right')
	ax[1].set_xlabel('$Time$ (min)')
	ax[1].set_ylabel('$E_{11}$')
	#
	#time-strain
	#ax[2].set_xlim(0.0, 0.05)
	#ax[2].set_ylim(0.0, 0.012)
	#ax[2].plot(-E_11_ana,-E_e_ana,ls='-',label='Analytical',color='b')#,marker = 'o')
	#ax[2].plot(-sol.get_strain().T[0],sol_E_e,marker='*',label='$\dot{\lambda}$ = -0.0005 /s',ls='',color='r')
	#ax[2].plot(sol2.get_time(),-sol2.get_strain().T[0],marker='*',label='$\dot{\lambda}$ = -0.0001/s',ls='',color='y')
	#ax[2].plot(sol3.get_time(),sol3.get_stress().T[0],ls='-.',label='$\lambda$ = 0.95',color='b')
	#commonFormat(ax[2],0.,0.,'lower right')
	#ax[2].set_xlabel('$E_{11}$')
	#ax[2].set_ylabel('$E_{11}^e$')
...
def set_material_ana(stretch,C1,pStretch,C1p,cp):
	material_ana = ana_elastoplastic(stretch,C1,pStretch,C1p,cp)
	return material_ana
...
from Classes.analy import ana_elastoplastic
...
#load = lc.loading_unloading(1.,0.95,-0.0166221,0.0166221)
load = lc.loading_unloading(1.,0.95,-0.0005,0.0005)
#load2 = lc.loading_unloading(1.,0.95,-0.0001,0.0001)
...
#material1 = set_material(0.0001,0.0001,0.00,0.005,0.005,0.00,15.,0.0001,0.0006,0.005,10.,0.04,0.005,0.005,50.)

# material parameter for analytical solution
material1 = set_material(43.0,0.0,0.00,0.0001,0.0,0.00,0.001,0.0,0.0,0.0,0.001,50.0,0.0,0.0,1.0)
...
stretch_ana = 0.95
material_ana = set_material_ana(stretch_ana,43.0,1.0,50.0,1.0)
#print('analytical', len(E_11_ana),len(t_ana)) # debugging

Sig_11_ana = material_ana.pk_11(stretch_ana,-0.0005,0.0005)
time_ana = material_ana.get_time()
E_11_ana = material_ana.get_GL11()
E_e_ana = material_ana.get_eps_e()
print('ana',time_ana)
...
#sol2 = global_Newton_delta(load2,material1)
print('num',sol.get_time())

#plot_all(sol1,sol2,sol3,"para_lambda.pdf",1)
...
plot_all(sol,"ch6_fig_epverification.pdf",1)

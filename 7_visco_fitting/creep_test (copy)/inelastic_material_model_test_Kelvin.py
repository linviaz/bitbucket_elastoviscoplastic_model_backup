...
def plot_all(sol,sol2,sol3,sol4,name,index=0):
	#Plot results
	setupMatplotlib(5.,4.)
	...
	#ax[0].plot(-sol.get_strain().T[0],-sol.get_stress().T[0],label='Numerical ',ls='--',color='b')
	#ax[0].plot(-sol2.get_strain().T[0],-sol2.get_stress().T[0],label='Numerical slower',ls='-',color='g')
	...
	# time-stress
	#ax[1].set_xlim(0.0, 1200.)
	#ax[1].set_ylim(-10, 30)
	#ax[0].plot(exp_time,exp_stress,ls='-',label='Experimental',color='r')#,marker = 'o')
	#ax[0].plot(sol.get_time(),-sol.get_stress().T[0],ls='--',label='Numerical',color='b')
	#ax[1].plot(sol2.get_time(),-sol2.get_stress().T[0],ls='--',label='Numerical slower',color='g')
	#ax[1].plot(sol3.get_time(),-sol3.get_stress().T[0],ls='--',label='$C_1$ = 30.0',color='b')
	#commonFormat(ax[0],0.,0.,'lower right')
	...
	#ax.plot(np.log(exp_time),-exp_strain11,ls='-',label='Experimental',color='r')#,marker = 'o')
	ax.plot(sol.get_time(),-sol.get_strain().T[0],ls='--',label='Cp = 0.0',color='b')
	ax.plot(sol2.get_time(),-sol2.get_strain().T[0],ls='--',label='Cp = 1',color='g')
	ax.plot(sol3.get_time(),-sol3.get_strain().T[0],ls='--',label='Cp = 0.1',color='r')
	ax.plot(sol4.get_time(),-sol4.get_strain().T[0],ls='--',label='Cp = 0.01',color='y')
	#
	#ax[2].plot(sol2.get_time(),-sol2.get_strain().T[0],ls='--',label='Numerical slower',color='g')
	#ax[2].plot(sol3.get_time(),-sol3.get_strain().T[0],ls='-.',label='C_1 = 5.0',color='b')
	commonFormat(ax,0.,0.,'upper right') #[0]
	ax.set_xlabel('$Time$')
	ax.set_ylabel('$E_{11}$')
...
#load2 = lc.relax(2.)
#load3 = lc.relax(3.)
ld = lc.experiment_creep(1.)
#load2 = lc.experiment_ld(10.)
...
material1 = set_material(0.001,0.001,0.0, 0.0001,0.0001,0.0,0.0001,0.0001,0.0001,0.0,0.0001, 70., 500., 00., 0.0)
material2 = set_material(0.001,0.001,0.0, 0.0001,0.0001,0.0,0.0001,0.0001,0.0001,0.0,0.0001, 70., 500., 00., 1.)
material3 = set_material(0.001,0.001,0.0, 0.0001,0.0001,0.0,0.0001,0.0001,0.0001,0.0,0.0001, 70., 500., 00., 0.1)
material4 = set_material(0.001,0.001,0.0, 0.0001,0.0001,0.0,0.0001,0.0001,0.0001,0.0,0.0001, 70., 500., 00., 0.01)

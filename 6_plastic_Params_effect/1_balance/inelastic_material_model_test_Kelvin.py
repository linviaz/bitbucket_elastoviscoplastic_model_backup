...
def plot_all(sol,name,index=0):
	#Plot results
	setupMatplotlib(14.,4.)
	fig, ax = plt.subplots(nrows=1,ncols=3)
	plt.subplots_adjust(wspace=0.5)
	sol_plastic_strain = 0.5*(sol.get_inelastic_tens().T[0,2]-1.)
	#sol2_plastic_strain = 0.5*(sol2.get_inelastic_tens().T[0,2]-1.)
	#sol3_plastic_strain = 0.5*(sol3.get_inelastic_tens().T[0,2]-1.)
	#print(0.5*(sol.get_inelastic_tens().T[0,2]-1.0)-sol.get_strain().T[0])
	
	#stress-strain
	#ax[0].set_xlim(0.0, -0.05)
	#ax[0].set_ylim(0.10, -0.35)
	#print(E_11_ana-sol.get_strain().T[0])
	ax[0].plot(-exp_strain11,exp_stress,ls='-',label='Experimental',color='r')#,marker = 'o')
	ax[0].plot(-sol.get_strain().T[0],-sol.get_stress().T[0],label='$C_1$ = 8 $C_{1p}$ =70 ',ls='--',color='b')
	#ax[0].plot(-sol2.get_strain().T[0],-sol2.get_stress().T[0],label='$C_1$ = 10 $C_{1p}$ =65',ls='--',color='g')
	#ax[0].plot(-sol3.get_strain().T[0],-sol3.get_stress().T[0],label='$C_1$ = 13 $C_{1p}$ =60',ls='--',color='y')
	#
	...
	#ax[1].set_ylim(0.10, -0.35)
	ax[1].plot(exp_time,exp_stress,ls='-',label='Experimental',color='r')#,marker = 'o')
	ax[1].plot(sol.get_time(),-sol.get_stress().T[0],ls='--',label='$C_1$ = 8 $C_{1p}$ =70',color='b')
	#ax[1].plot(sol2.get_time(),-sol2.get_stress().T[0],ls='--',label='$C_1$ = 10 $C_{1p}$ =65',color='g')
	#ax[1].plot(sol3.get_time(),-sol3.get_stress().T[0],ls='--',label='$C_1$ = 13 $C_{1p}$ =60',color='y')
	#
	...
	#ax[2].set_ylim(0.0, -0.05)
	#ax[2].plot(exp_time,-exp_strain,ls='-',label='Experimental',color='b')#,marker = 'o')
	ax[2].plot(-sol.get_strain().T[0],-sol_plastic_strain,ls='-',label='$C_1$ = 8 $C_{1p}$ =70',color='b')
	#ax[2].plot(-sol2.get_strain().T[0],-sol2_plastic_strain,ls='-',label='$C_1$ = 10 $C_{1p}$ =65',color='g')
	#ax[2].plot(-sol3.get_strain().T[0],-sol3_plastic_strain,ls='-',label='$C_1$ = 13 $C_{1p}$ =60',color='y')
	#ax[2].plot(sol2.get_time(),-sol2.get_strain().T[0],ls='--',label='C_1 = 3.0',color='g')
	#ax[2].plot(sol3.get_time(),-sol3.get_strain().T[0],ls='-.',label='C_1 = 5.0',color='b')
	#commonFormat(ax[2],0.,0.,'lower right')
	ax[2].set_xlabel('$E_{11}$')
	ax[2].set_ylabel('$E_{11}^p$')
...
def global_Newton_delta(load,material,name="solution_delta"):
	#for boundary conditions/global NR algorithm:
	loading_scheme = load[0][3]
	# for unloading tensile stress, define a second strain array
	stress_control = ['s','s','s','s','s','s']	
	#set up solution tracker
	...
	for i in range(1,len(load)):
		...
		#Stress-control for unloading tensile
		pos_tensile_e = get_control_positions(stress_control,'e')
		pos_tensile_s = get_control_positions(stress_control,'s')
		#
		#determine load increment vector
		...
		while ((load[i][0] - local_time) >= DBL_EPS):
			...
			hist_tens_t = np.array(sol.get_inelastic_tens()[len(sol.get_inelastic_tens())-1])
			hist_scal_t = np.array(sol.get_inelastic_scal()[len(sol.get_inelastic_scal())-1])
			#define strain for unloading: during loading, eps_unloading=eps_tdt; during unloading, eps_unloading=f(sig_t)
			eps_unloading = np.array(sol.get_strain()[len(sol.get_strain())-1] )#added for tensile check
			#
			#
			if (sig_int[0] > 0.0): 
				for k in range(6):
					if(load[i][3][k] == 'e'):
						eps_tdt[k] = eps_t[k] + incr[k]
					sig_ext[k] += 0.0
				#sig_int = material.calc_new_stress(dt,eps_t,eps_unloading,sig_t,deps_dt_t,hist_tens_t,hist_scal_t)
				residual = sig_ext-sig_int
				#
				#
				residual_tensile = np.delete(residual,pos_tensile_e,0)
				residual = np.delete(residual,pos_e,0)
				#print('residual_tensile',residual_tensile)
				eps_inc = 1.*residual
				eps_inc_tensile = 1.*residual_tensile
				iter = 0
				while (not (convergence_check(residual_tensile,eps_inc_tensile,1.e-12)) and iter < ITER_MAX):
					iter += 1
					stiffn = material.get_dsig_dE()
					#
					#build global Jacobian out of stiffness matrix
					...
					#the solution vector is zero (no increment, since we have the solution already)
					#K = np.delete(np.delete(stiffn,pos_e,1),pos_e,0)
					K_tensile = np.delete(np.delete(stiffn,pos_tensile_e,1),pos_tensile_e,0)
					#print('k_tensile',K_tensile)
					#get increment
					#eps_inc = np.linalg.solve(K,residual)
					eps_inc_tensile = np.linalg.solve(K_tensile,residual_tensile)
					#increment strain only where no bc applied
					#for j in range(len(eps_inc)):
					#		eps_tdt[pos_s[j]] += eps_inc[j]
					for k in range(len(eps_inc_tensile)):
							eps_unloading[pos_tensile_s[k]] += eps_inc_tensile[k]
					#perform stress integration
					if (eps_unloading[0]<eps_tdt[0]):
						sig_int = material.calc_new_stress(dt,eps_t,eps_unloading,sig_t,deps_dt_t,hist_tens_t,hist_scal_t)
					else:
						sig_int = material.calc_new_stress(dt,eps_t,eps_tdt,sig_t,deps_dt_t,hist_tens_t,hist_scal_t)
					residual = sig_ext-sig_int
			...
			#
				...
				for j in range(6):
					if (load[i][3][j] == 'e'):
						eps_tdt[j] = eps_t[j] + incr[j] #put in bc and determine total strain
						eps_unloading[j]= eps_t[j] + incr[j] 
					else:
						sig_ext[j] += incr[j] #external stress increased by driving force
				sig_int = material.calc_new_stress(dt,eps_t,eps_tdt,sig_t,deps_dt_t,hist_tens_t,hist_scal_t)
				residual = sig_ext-sig_int
				...
				while (not (convergence_check(residual,eps_inc,1.e-12)) and iter < ITER_MAX):
					...
					for j in range(len(eps_inc)):
							eps_tdt[pos_s[j]] += eps_inc[j]
							eps_unloading[pos_s[j]] += eps_inc[j]
					#perform stress integration
					sig_int = material.calc_new_stress(dt,eps_t,eps_tdt,sig_t,deps_dt_t,hist_tens_t,hist_scal_t)
					residual = sig_ext-sig_int
					residual = np.delete(residual,pos_e,0)
...
material1 = set_material(4.,500.0,0.0,  0.0001,0.0001,0.0,0.001,0.0001,0.0001,0.0,0.001, 60., 0.01, 0., 0.03)
#material2 = set_material(0.001,500.0,0.0,  0.0001,0.0001,0.0,0.001,0.0001,0.0001,0.0,0.001, 30., 0.01, 0., 0.01)
#material3 = set_material(1.,  500.0,0.0,  0.0001,0.0001,0.0,0.001,0.0001,0.0001,0.0,0.001, 50., 0.01, 0., 0.01)
...
plot_all(sol1,"ch6_para_balance.pdf",1)

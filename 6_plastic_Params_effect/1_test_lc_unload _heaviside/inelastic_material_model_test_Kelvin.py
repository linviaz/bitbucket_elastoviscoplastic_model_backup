...
def plot_all(sol,sol2,name,index=0):
	#Plot results
	...
	ax[0].plot(-exp_strain11,-sol.get_stress().T[0],label='Modified',ls='-',color='b') #sol.get_strain().T[0]
	ax[0].plot(-exp_strain11,-sol2.get_stress().T[0],label='Original',ls='--',color='y')  #sol2.get_strain().T[0]
	#ax[0].plot(-sol3.get_strain().T[0],-sol3.get_stress().T[0],label='$C_1$ = 30.0',ls='--',color='b')  # sol2.get_strain().T[0]
	...
	ax[1].plot(sol.get_time(),-sol.get_stress().T[0],ls='-',label='Modified',color='b')
	ax[1].plot(sol2.get_time(),-sol2.get_stress().T[0],ls='--',label='Original',color='y')
	...
	#ax[2].plot(exp_time,-exp_strain11,ls='-',label='Experimental',color='b')#,marker = 'o')
	#ax[2].plot(sol.get_time(),-sol.get_strain().T[0],ls='-',label='C_1 = 0.5',color='r')
	#ax[2].plot(-sol.get_strain().T[0],sol_elastic_strain,ls='-',label='Elastic',color='b')#,marker = 'o')
	ax[2].plot(-sol.get_strain().T[0],sol_plastic_strain,ls='-',label='Modified',color='b')
	ax[2].plot(-sol2.get_strain().T[0],sol2_plastic_strain,ls='--',label='Original',color='y')
	#ax[2].plot(sol3.get_time(),-sol3.get_strain().T[0],ls='-.',label='C_1 = 5.0',color='b')
	commonFormat(ax[2],0.,0.,'upper left')
...
def set_material2(C1,D2,alpha,C1v,D2v,alphav,eta,C1v2,D2v2,alphav2,eta2,C1p,D2p,alphap,cp):
	#choose material
	material2 = viscoelastoplastic2(C1,D2,alpha,C1v,D2v,alphav,eta,C1v2,D2v2,alphav2,eta2,C1p,D2p,alphap,cp)#Parameters in MPa
	return material2	
...
from Classes.Minkley_CS2 import viscoelastoplastic2
...
material1 = set_material(9.0,500.0,0.0,  8.0,500.,0.0,13000.,40.,500.0,0.,500, 70., 500., 00., 0.1)
material2 = set_material2(9.0,500.0,0.0,  8.0,500.,0.0,13000.,40.,500.0,0.,500, 70., 500., 00., 0.1)
...
plot_all(sol1,sol2,"ch6_para_ep_2mater.pdf",1)

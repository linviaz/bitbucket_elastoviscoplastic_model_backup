...
def plot_all(sol,name,index=0):
	...
	# time-stress
	#ax[1].set_xlim(0.0, 7000.)
	#ax[1].set_ylim(-10, 20)
	ax[1].plot(exp_time,exp_stress,ls='-',label='Experimental',color='r')#,marker = 'o')
	ax[1].plot(sol.get_time(),-sol.get_stress().T[0],ls='--',label='Numerical',color='b')
	...
	ax[2].plot(exp_time,-exp_strain11,ls='-',label='Experimental',color='r')#,marker = 'o')
	ax[2].plot(sol.get_time(),-sol.get_strain().T[0],ls='--',label='Numerical',color='b')
...
plot_all(sol1,"ch6_2V9_global.pdf",1)
